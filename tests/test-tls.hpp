...
struct FIXTURE_NAME: public default_sample {
	...
	iostream_sptr baseStream;
	TilesetPtr pTileset;
	MP_SUPPDATA suppData;
	//std::map<TilesetType::E_SUPPTYPE, sstr_ptr> suppBase;
...
ISINSTANCE_TEST(c00, INITIALSTATE_NAME, TilesetType::PossiblyYes);
/*
BOOST_AUTO_TEST_CASE(TEST_NAME(get_images))
{
	BOOST_TEST_MESSAGE("Extracting first image in first tileset");

	TilesetPtr nextTileset = this->pTileset->getTileset(0);
	ImagePtr img = nextTileset->openImage(0);
	...
	nextTileset = this->pTileset->getTileset(0);
...
BOOST_AUTO_TEST_CASE(TEST_NAME(overwrite_image))
{
...

	{
		...
		TilesetPtr nextTileset = this->pTileset->getTileset(0);
#else
		TilesetPtr nextTileset = this->pTileset;
#endif
		ImagePtr img = nextTileset->openImage(0);
...
BOOST_AUTO_TEST_CASE(TEST_NAME(append_tileset))
{
	...
	{
		TilesetPtr newTileset = this->pTileset->insertTileset(0);
		assert(newTileset);

		newTileset->setImageCount(1);
		ImagePtr img = newTileset->openImage(0);
		assert(img);
